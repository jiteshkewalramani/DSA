...
export default function LearningSection({ learningContent }) {
  ...
  const renderIntroduction = () => (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-blue-100 to-purple-100 dark:from-blue-900 dark:to-purple-900 rounded-lg p-6 border border-blue-200 dark:border-blue-700">
      ...
      {learningContent.introduction.visualExample && (
        <div className="bg-slate-100 dark:bg-slate-800 rounded-lg p-6 border border-slate-200 dark:border-slate-600">
          ...
          <p className="text-slate-600 dark:text-slate-300 mb-4">{learningContent.introduction.visualExample.description}</p>
          <pre className="bg-slate-200 dark:bg-slate-950 rounded p-4 text-green-700 dark:text-green-400 font-mono text-sm overflow-x-auto">
    ...
    return (
      <div className="space-y-4">
        ...
        {(sectionData.sections || []).map((section, idx) => (
          <div key={idx} className="bg-slate-100 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-600 overflow-hidden">
            <button
              onClick={() => toggleCard(`fund-${idx}`)}
              className="w-full px-6 py-4 flex items-center justify-between hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
            >
              <h3 className="text-lg font-semibold text-slate-800 dark:text-white">{section.subtitle || section.concept || section.name || section.title || 'Section'}</h3>
              {expandedCards[`fund-${idx}`] ? <ChevronUp size={20} className="text-slate-500 dark:text-slate-400" /> : <ChevronDown size={20} className="text-slate-500 dark:text-slate-400" />}
            </button>
            ...
            {expandedCards[`fund-${idx}`] && (
              <div className="px-6 pb-6 space-y-4">
                ...
                {(section.example || section.diagram) && (
                  <div className="bg-slate-200 dark:bg-slate-950 rounded p-4 border-l-4 border-blue-500">
                    <pre className="text-sm text-blue-700 dark:text-blue-300 font-mono whitespace-pre-wrap">
                      {section.example || section.diagram}
                    </pre>
                  </div>
                )}
          ...
          <div key={idx} className="bg-slate-800 rounded-lg border border-slate-600 overflow-hidden">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className={`px-2 py-1 rounded text-xs font-semibold ${problem.difficulty === 'Easy' ? 'bg-green-900/50 text-green-300' :
  ...
  return (
    <div>
      {/* Header Section */}
      <div className="bg-gradient-to-r from-blue-100 to-purple-100 dark:from-blue-900/30 dark:to-purple-900/30 border-b border-slate-200 dark:border-slate-700 px-4 md:px-6 lg:px-8 py-6 md:py-8 mb-6 md:mb-8">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold text-slate-800 dark:text-white mb-2 flex items-center gap-2 md:gap-3">
            <BookOpen className="text-blue-600 dark:text-blue-400" size={28} md-size={32} lg-size={36} />
            Complete Learning Guide
          </h1>
          <p className="text-slate-600 dark:text-slate-300 text-sm md:text-base lg:text-lg">Master the concepts with comprehensive theory and examples</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8">
            ...
            {sections.map(section => {
              ...
              return (
                <button
                  ...
                  className={`flex-shrink-0 px-4 md:px-5 py-2.5 md:py-3 rounded-lg flex items-center gap-2 md:gap-2.5 transition-all font-medium text-sm md:text-base ${activeSection === section.id
                    ? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg shadow-blue-500/30 md:scale-105'
                    : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white border border-slate-300 dark:border-slate-700 shadow-sm'
                    }`}
                >
        ...
        {/* Content Area */}
        <div className="bg-white/80 dark:bg-slate-900/50 rounded-lg md:rounded-xl p-4 md:p-6 lg:p-8 border border-slate-200 dark:border-slate-700 shadow-xl dark:shadow-2xl mb-6 md:mb-8">
          {renderSection()}
        </div>
      </div>
    </div>
